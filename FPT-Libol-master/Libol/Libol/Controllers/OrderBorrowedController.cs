    ...
    public class OrderBorrowedController : Controller
    {
		...
		public JsonResult sendMail(string strOrderID)
		{
			...
			foreach (string item in orderArr)
			{
				...
				sendmail(patron.Email, name, date);
				le.ExcuteSQL("update CIR_HOLDING set CheckMail=1, InTurn=1 where id =" + item);
			}
		...
		private void sendmail(string email,string name,string timeout)
		{
