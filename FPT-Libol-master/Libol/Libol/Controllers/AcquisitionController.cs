...
namespace Libol.Controllers
{
	public class AcquisitionController : Controller
	{
		...
		public PartialViewResult GetInventoryReport(string strInventoryID, string strLibID, string strDKCBID)
		{
			...
			List<FPT_SP_INVENTORY_Result> listDataTemp = ab.FPT_SP_INVENTORY(libid);
			if (myList.Length != 0)
			{
		...
		public PartialViewResult RecordNotFound(string strInventoryID, string strLibID, string strDKCBID)
		{
			strDKCBID = strDKCBID.Trim();
			string[] myList = strDKCBID.Split('\n');
			int countCN = myList.Length;
			...
			if (myList.Length != 0)
			{
				...
				for (int j = 0; j < listData.Count; j++)
				{
					if (listData[j].CopyNumber.Equals("GT/CNTT002011"))
					{
						int a = 6;
					}
					for (int i = 0; i < listStr.Count; i++)
