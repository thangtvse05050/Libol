...
GO
/****** Object:  StoredProcedure [dbo].[FPT_CHECK_DATA_EXCEL_NULL]    Script Date: 12/3/2019 9:36:25 PM ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
...
AS
	  ...
	  GO
/****** Object:  StoredProcedure [dbo].[FPT_GET_DOB_EXCEL]    Script Date: 12/3/2019 9:37:52 PM ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
	  CREATE procedure [dbo].[FPT_GET_DOB_EXCEL]
	  AS
	select DOB from PATRON_BY_EXCEL where DOB not in (Select  DOB from PATRON_BY_EXCEL where DOB='')
GO
/****** Object:  StoredProcedure [dbo].[FPT_GET_DATE_FAIL]    Script Date: 12/3/2019 9:39:08 PM ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
	CREATE PROCEDURE [dbo].[FPT_GET_DATE_FAIL](@stringDOB NVARCHAR(MAX))
	AS
	SELECT * from PATRON_BY_EXCEL where @stringDOB like '%;'+cast(DOB as varchar(200))+';%'	
	GO
/****** Object:  StoredProcedure [dbo].[FPT_GET_CODE_FAIL]    Script Date: 12/3/2019 9:40:02 PM ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
	CREATE PROCEDURE [dbo].[FPT_GET_CODE_FAIL](@stringCode NVARCHAR(MAX))
	AS
	SELECT * from PATRON_BY_EXCEL where @stringCode like '%;'+cast(Code as varchar(200))+';%'
	GO
/****** Object:  StoredProcedure [dbo].[FPT_GET_EMAIL_FAIL]    Script Date: 12/3/2019 9:40:46 PM ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
	CREATE PROCEDURE [dbo].[FPT_GET_EMAIL_FAIL](@stringEmail NVARCHAR(MAX))
	AS
	SELECT * from PATRON_BY_EXCEL where @stringEmail like '%;'+cast(Email as varchar(200))+';%'	
	GO
/****** Object:  StoredProcedure [dbo].[FPT_SELECT_DUPLICATES_CODE]    Script Date: 12/3/2019 9:41:36 PM ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
	CREATE PROCEDURE [dbo].[FPT_SELECT_DUPLICATES_CODE]
AS
SELECT a.*
...
ON 
 ...
 GO
/****** Object:  StoredProcedure [dbo].[FPT_SELECT_DUPLICATES_EMAIL]    Script Date: 12/3/2019 9:41:42 PM ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
	CREATE PROCEDURE [dbo].[FPT_SELECT_DUPLICATES_EMAIL]
AS
SELECT a.*
FROM PATRON_BY_EXCEL a
...
GO
/****** Object:  StoredProcedure [dbo].[FPT_GET_FACULTY_FAIL_DETAIL]    Script Date: 12/3/2019 9:45:23 PM ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
CREATE PROCEDURE [dbo].[FPT_GET_FACULTY_FAIL_DETAIL](@strCollege VARCHAR(MAX))
AS
select * from PATRON_BY_EXCEL where id not in (select c.ID from PATRON_BY_EXCEL c join CIR_DIC_FACULTY a  join CIR_DIC_COLLEGE b on a.CollegeID=b.ID on c.Faculty=a.Faculty and c.College=b.College where @strCollege like '%;'+cast(c.College as varchar(200))+';%')	and Faculty not like '' and College not like ''

GO
/****** Object:  StoredProcedure [dbo].[FPT_SP_Z3950_GET_HOST_PORT_DBNAME]    Script Date: 12/5/2019 7:42:46 PM ******/
-- Create: DungPT
--Date: 05/12/2019
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
CREATE PROCEDURE [dbo].[FPT_SP_Z3950_GET_HOST_PORT_DBNAME]
AS
	SELECT A.ID, B.ID AS DatabaseID, A.[Name], A.Host, A.[Port], A.Account, A.[Password], A.PreferredLender,
	B.DBName, B.[Description] FROM ILL_Z3950_SERVERS AS A, ILL_Z3950_DBS AS B
	WHERE A.ID = B.ServerID


GO
/****** Object:  StoredProcedure [dbo].[FPT_SP_Z3950_EDIT_SERVER]    Script Date: 12/5/2019 7:46:12 PM ******/
-- Create: DungPT
--Date: 05/12/2019
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
CREATE procedure [dbo].[FPT_SP_Z3950_EDIT_SERVER]
(
	@intID int,
	@strName nvarchar(50),
	@strHost nvarchar(50),
	@intPort int,
	@strAccount varchar(32),
	@strPassword varchar(32),
	@strDBName varchar(20),
	@strDescription nvarchar(200)
)
AS
	Update ILL_Z3950_SERVERS 
	SET [Name] = @strName, Host = @strHost, [Port] = @intPort, Account = @strAccount, [Password] = @strPassword
	Where ID = @intID

	Update ILL_Z3950_DBS 
	SET DBName = @strDBName, [Description] = @strDescription
	Where ServerID = @intID

	GO
/****** Object:  StoredProcedure [dbo].[FPT_SP_Z3950_DELETE_SERVER]    Script Date: 12/5/2019 7:47:01 PM ******/
-- Create: DungPT
--Date: 05/12/2019
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
CREATE PROCEDURE [dbo].[FPT_SP_Z3950_DELETE_SERVER]
(
	@intID int
)
AS
 DELETE FROM ILL_Z3950_DBS WHERE ServerID = @intID
 DELETE FROM ILL_Z3950_SERVERS WHERE ID = @intID

 GO
/****** Object:  StoredProcedure [dbo].[FPT_SP_Z3950_ADD_NEW_SERVER]    Script Date: 12/5/2019 7:47:25 PM ******/
-- Create: DungPT
--Date: 05/12/2019
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
CREATE procedure [dbo].[FPT_SP_Z3950_ADD_NEW_SERVER]
(
	@strName nvarchar(50),
	@strHost nvarchar(50),
	@intPort int,
...
AS
	Declare @intSID int;
	Declare @intServerID int;
	Declare @intDbsID int;
	set @intServerID = (SELECT MAX(ID) FROM ILL_Z3950_SERVERS) + 1;
	set @intDbsID = (SELECT MAX(ID) FROM ILL_Z3950_DBS) + 1; 
	Insert into ILL_Z3950_SERVERS(ID, [Name], Host, [Port], Account, [Password], PreferredLender) values( @intServerID ,@strName, @strHost, @intPort, @strAccount, @strPassword, 1)

	set @intSID = ( SELECT MAX(ID) FROM ILL_Z3950_SERVERS);

	Insert into ILL_Z3950_DBS(ID, ServerID, DBName,[Description]) values(@intDbsID ,@intSID , @strDBName, @strDescription)

	GO
/****** Object:  StoredProcedure [dbo].[FPT_SP_GET_SERVERS_Z3950]    Script Date: 12/5/2019 7:47:59 PM ******/
-- Create: DungPT
--Date: 05/12/2019
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
CREATE Procedure [dbo].[FPT_SP_GET_SERVERS_Z3950]

AS
	Select S.ID, S.[Name], S.Host, S.[Port], S.Account, S.[Password], D.DBName, D.[Description]
	from ILL_Z3950_SERVERS S, ILL_Z3950_DBS D
	where S.ID = D.ServerID
	go
	----------------------
...
DECLARE @lngID 	INT  
	 ...
	 BEGIN
		...
		SELECT @intLoanTypeID=HOLDING.loantypeid,@intLocationID=Holding.locationid,@intItemID=Holding.itemid FROM HOLDING WHERE UPPER(CopyNumber)=UPPER(@strCopyNumber)
		SELECT @intTimeUnit=TimeUnit,@intLOANPERIOD=LOANPERIOD FROM CIR_LOAN_TYPE WHERE ID=@intLoanTypeID
		...
		SELECT @intLoanTypeID=HOLDING.loantypeid,@intLocationID=Holding.locationid,@intItemID=Holding.itemid FROM HOLDING WHERE UPPER(CopyNumber)=UPPER(@strCopyNumber)
	 -- Get NextID   
	 SELECT @lngID = ISNULL(MAX(ID), 0) + 1 FROM CIR_LOAN  
	 SET @intOutID = @lngID  
  
 	 -- Get PatronID  
 	 SELECT @lngPatronID = ID FROM CIR_PATRON WHERE UPPER(Code) = UPPER(@strPatronCode)  
 	 
	 -- Create Loan record  
	 --IF (@strDueDate = 'NULL' OR @strDueDate = ''  ) and (@intUserID=0)
	 IF (@strDueDate='ISNULL') -- Han tra mo
  	 	INSERT INTO CIR_LOAN (ID, ItemID, CopyNumber, CheckOutDate, LoanMode, LoanTypeID, PatronID, LocationID) VALUES (@lngID, @intItemID, UPPER(@strCopyNumber), @strCheckOutDate,  @intLoanMode, @intLoanTypeID, @lngPatronID, @intLocationID)  
 	 ELSE  
  	 BEGIN  
		-- Ngay tra duoc nhap vao
	 	IF @strDueDate <> '' AND CONVERT(DATETIME,@strDueDate) > CONVERT(DATETIME,@strCheckOutDate)
	  	 	INSERT INTO CIR_LOAN (ID, ItemID, CopyNumber, CheckOutDate, DueDate, LoanMode, LoanTypeID, PatronID, LocationID) VALUES (@lngID, @intItemID, UPPER(@strCopyNumber), @strCheckOutDate, @strDueDate, @intLoanMode, @intLoanTypeID, @lngPatronID, @intLocationID)  
		ELSE
	 ...
	 -- Update Copynumber' status  
	 UPDATE HOLDING SET InUsed = 1, OnHold = 0, UseCount = UseCount + 1, DateLastUsed = GetDate() WHERE UPPER(CopyNumber) = UPPER(@strCopyNumber)  
	 -- Calculation FreeCopies  
	 UPDATE HOLDING_COPY SET FreeCopies = FreeCopies - 1 WHERE ItemID = @intItemID  
	 -- Delete the request record if it exist  
...
SET QUOTED_IDENTIFIER ON
GO
Create PROCEDURE  [dbo].[FPT_ADD_CITY]
@intCityID int, @City nvarchar(200)
